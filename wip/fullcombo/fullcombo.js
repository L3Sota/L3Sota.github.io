...
var key_pos = [graph_width * 0.9, graph_height * 0.0];
var types = [
...
function plot(data) {
  ...
  for (var i = 0; i < types.length; i++) {
    ...
    key
      ...
      .attr(
        "transform",
        "translate(" + key_offset_x[0] + "," + key_offset_y_i + ")"
      )
      .append("text")
      .attr(
        "transform",
        "translate(" + key_offset_x[1] + "," + key_offset_y_i + ")"
      )
      .text(function (d) {
        return types[i].symbol;
      });
  }
  ...
  points
    ...
    .classed("all", function (d) {
      return "all" == types[d.type].type;
    })
    .classed("cute", function (d) {
      return "cute" == types[d.type].type;
    })
    .classed("cool", function (d) {
      return "cool" == types[d.type].type;
    })
    .classed("passion", function (d) {
      return "passion" == types[d.type].type;
    })
    ...
    .style("fill", function (d) {
      console.log(scale_c(d.type));
      return scale_c(d.type);
    });
}
...
function highlight(d, i) {
  ...
  if (moused) {
    ...
    if (deletable) {
      d3.select(this).selectAll(".popups").remove();
    }
  }
}
